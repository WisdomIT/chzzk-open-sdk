...
| 2   | 🟡         | 토큰 응답 `expiresIn`                           | Type: **String** ("86400")                                      | 타입도 `string`으로 정의됨                            | **SDK(#5)는 string/number 모두 수용해 number로 정규화** (`ChzzkTokenSet.expiresIn`). 실제 JSON 타입은 하니스에서 실측       |
| 3   | 🟡         | 토큰 발급 응답 `scope`                          | 발급(authorization_code) 응답엔 없음, 갱신(refresh) 응답엔 있음 | wizbot 타입엔 항상 있음                               | **SDK(#5)는 optional 처리** — 있으면 보존, 없으면 생략. 발급/갱신 각각 실측 예정                                            |
