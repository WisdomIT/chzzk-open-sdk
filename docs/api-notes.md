...
| --- | ---------- | ----------------------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 1   | 🔴         | 토큰 폐기 경로                                  | `POST /auth/v1/token/revoke`                                    | `POST /auth/v1/revoke` 호출 (동작했던 것으로 추정)    | 두 경로 모두 실호출로 확인 후 문서 경로를 기본으로, 결과 여기 기록                    |
| 2   | 🔴         | 토큰 응답 `expiresIn`                           | Type: **String** ("86400")                                      | 타입도 `string`으로 정의됨                            | 실제 JSON 타입 확인 (숫자 의심). SDK는 정규화 계층에서 number로 통일 예정             |
| 3   | 🔴         | 토큰 발급 응답 `scope`                          | 발급(authorization_code) 응답엔 없음, 갱신(refresh) 응답엔 있음 | wizbot 타입엔 항상 있음                               | 발급/갱신 각각 실측. optional 처리 예정                                               |
